...
const buildMeasureIcon = (
  ...
  const buildText = (
    ...
    if (type === 'keystop') {
      ...
      text.fontName = !characters.includes(':') ? {family: typefaceToUse.family, style: "Regular"} : typefaceToUse;
    }
   ...
   * @param {FrameNode} icon A copy of the annotation image for reference.
   * @param {string} name The name of the annotation.
   *
   * @returns {Object} The final legend after positioning.
   * @private
   */
  const buildLegendEntry = (nodeData: PluginViewObject, icon: FrameNode, name: string) => {
    const { role, labels } = nodeData;
    let roleLabels;
    if (role === 'image-decorative') {
      roleLabels = {};
    } else if (role === 'image') {
      roleLabels = { 'Alt text': labels?.alt };
    } else {
      roleLabels = { 'Label (visible)': labels?.visible, 'Label (a11y)': labels?.a11y };
    }
  
    const legendItem: FrameNode = figma.createFrame();
    legendItem.name = name + ' Annotation';
    icon.name = 'Icon';
    icon.children.find(node => node.type === 'VECTOR').remove();
    ...
    legendItem.paddingTop = 10;
    legendItem.paddingBottom = 10;
    ...
    legendData.primaryAxisSizingMode = 'FIXED';
    ...
    legendData.layoutGrow = 0;
  
    // set padding and item spacing
    legendData.paddingLeft = 10;
    legendData.paddingRight = 6;
    legendData.paddingTop = 0;
    legendData.paddingBottom = 2;
    legendData.itemSpacing = 0;
  
    // add a line for each label
    Object.entries({'Role': role, ...roleLabels}).forEach(([key, value]: Array<any>) => {
